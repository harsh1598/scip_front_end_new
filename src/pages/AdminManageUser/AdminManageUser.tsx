import React, { useEffect, useRef, useState } from "react";
import {
    Col,
    Container,
    Row,
    CardHeader,
    Card,
    UncontrolledDropdown,
    DropdownToggle,
    DropdownMenu,
    DropdownItem,
    Input,
    Modal,
    ModalHeader,
    ModalFooter,
} from "reactstrap";
import { Link, useNavigate, useParams } from "react-router-dom";
import toast from "react-hot-toast";
import { ProjectName } from "../../Components/constants/layout";
import Grid, { GridColumn, GridRow } from "../../Components/Grid/Grid";
import WebService from "../../utility/WebService";
import BreadCrumb from "../../Components/Common/BreadCrumb";
import transfer from "../../../src/assets/images/transfer.svg"
import hanburger from "../../../src/assets/images/hamburger.svg"
import AddEditAdminManageUser from "./AddEditAdminManageUser";

const AdminManageUser = () => {
    document.title = `${ProjectName} | Manage User`;
    const [modelName, setModelName] = useState<any>("");
    const [data, setData] = useState({ alert: false, id: 0, status: "" });
    const [rows, setRows] = useState<GridRow[]>([]);
    const [totalCount, setTotalCount] = useState<any>(0);
    const [showloader, setShowLoader] = useState(false);
    const [headers, setHeaders] = useState<any[]>([]);
    const [searchText, setSearchText] = useState("");
    const formData = useRef({});
    const history = useNavigate();
    let { id } = useParams();

    useEffect(() => {
        getList(1);
    }, []);

    const onchange = (currentPage: number) => {
        getList(currentPage);
    };

    const getList = (page: any) => {
        setShowLoader(true);
        let obj: any = {};
        obj.offset = page - 1;
        if (searchText) {
            obj.keyword = searchText;
        }
        WebService.getAPI({
            action: `/syndicate-admin-user-pagination-list?parent_id=${id}`,
            body: obj,
        })
            .then((res: any) => {
                if (res && res.coloumns) {
                    var arr: any = [];
                    // SR NO
                    var obj: any = {
                        title: "Sr No.",
                        classTitle: "text-start",
                    };
                    arr.push(obj);

                    // REST FIELDS
                    for (var i in res.coloumns) {
                        var obj: any = {
                            title: res.coloumns[i].title,
                            db_column: res.coloumns[i].db_column,
                            control_type: res.coloumns[i].control_type,
                            nature: res.coloumns[i].nature,
                            source_value_name: res.coloumns[i].source_value_name,
                            classTitle: "text-start",
                        };
                        arr.push(obj);
                    }
                    // ACTION
                    var obj1: any = {
                        title: "Action",
                        classTitle: "text-start",
                    };
                    arr.push(obj1);
                    setHeaders(arr);
                }
                setRows([]);
                if (res && res.list) {
                    setTotalCount(res.count);
                    setShowLoader(false);
                    let rows: GridRow[] = [];
                    for (var i in res.list) {
                        let columns: GridColumn[] = [];
                        for (var j in arr) {
                            if (arr[j].title == "Sr No.") {
                                columns.push({
                                    value:
                                        page == 0
                                            ? Number(i) + 1
                                            : 10 * Number(page - 1) + Number(i) + 1,
                                });
                            } else if (arr[j].title == "Action") {
                                columns.push({
                                    value: actionList({
                                        id: res.list[i].userId,
                                        active: res.list[i].active,
                                    }),
                                });
                            } else if (
                                arr[j].control_type == "SELECT" &&
                                arr[j].nature == "DYNAMIC"
                            ) {
                                columns.push({ value: res.list[i][arr[j].source_value_name] });
                            } else {
                                columns.push({ value: res.list[i][arr[j].db_column] });
                            }
                        }
                        rows.push({ data: columns });
                        setRows([...rows]);
                    }
                }
            })
            .catch((error: any) => {
                setShowLoader(false);
            });
    };

    const onClickRoleAccess = (id: any) => {
        let data: any = {
            id: id,
        };
        WebService.getAccesstoken({
            action: "/check-user-role",
            body: data,
        })
            .then((res: any) => {
                if (res && res.user_role_id) {
                    history("/sub-user-role/permission/" + res.user_role_id);
                } else {
                    history("/sub-user-role/permission");
                }
            })
            .catch((error: any) => {
            });
    }

    const actionList = (data: any) => {
        return (
            <>
                {" "}
                <ul className="list-inline  gap-2 mb-0">
                    <li className="list-inline-item">
                        <UncontrolledDropdown>
                            <DropdownToggle
                                className="btn btn-soft-secondary btn-sm dropdown"
                                tag="button">
                                <i className="ri-more-fill align-middle"></i>
                            </DropdownToggle>
                            <DropdownMenu className="dropdown-menu-end">
                                <DropdownItem
                                    onClick={(e) => toggleModel("AddEditManageUser", data.id)}>
                                    <i className="ri-edit-box-line align-middle me-1"></i>Edit
                                </DropdownItem>
                                {data.active === "Y" && (
                                    <DropdownItem
                                        onClick={(e) =>
                                            setData({
                                                alert: true,
                                                id: data.id,
                                                status: "N",
                                            })
                                        }>
                                        <i className="ri-close-circle-line align-middle me-1" style={{ color: "red" }}></i>
                                        Deactive
                                    </DropdownItem>
                                )}
                                {data.active === "N" && (
                                    <DropdownItem
                                        onClick={(e) =>
                                            setData({
                                                alert: true,
                                                id: data.id,
                                                status: "Y",
                                            })
                                        }>
                                        <i className="ri-checkbox-circle-line align-middle me-1" style={{ color: "green" }}></i>
                                        Active
                                    </DropdownItem>
                                )}
                                <DropdownItem>
                                    <Link
                                        className="text-body"
                                        id="Team"
                                        to="/admin/TeamMember">
                                        <img src={hanburger} />
                                        Module Column Access
                                    </Link>
                                </DropdownItem>
                                <DropdownItem>
                                    <a
                                        className="text-body"
                                        id="Team"
                                        onClick={() => {
                                            onClickRoleAccess(data.id)
                                        }}
                                    >
                                        Role Access
                                    </a>
                                </DropdownItem>
                                <DropdownItem>
                                    <Link
                                        className="text-body"
                                        id="Team"
                                        to="/admin/TeamMember">
                                        <img src={transfer} />
                                        Transfer data to primary user
                                    </Link>
                                </DropdownItem>
                            </DropdownMenu>
                        </UncontrolledDropdown>
                    </li>
                </ul>
            </>
        );
    };

    const toggleModel = (name: string, id?: any) => {
        if (id) {
            WebService.getAPI({
                action: "/syndicate-admin-user-details/" + id,
            })
                .then((res: any) => {
                    if (res && res.result) {
                        formData.current = res.result;
                        setModelName(name);
                    }
                })
                .catch((error: any) => { });
        } else {
            formData.current = "";
            setModelName(name);
        }
    };

    const changeStatus = (id: any, status?: any) => {
        let obj: any = {};
        obj.userId = id;
        WebService.postAPI({
            action: `/syndicate-admin-user-update-status`,
            body: obj,
            id: "manage-user-modal-submit-btn",
        })
            .then((res: any) => {
                if (res) {
                    toast.success(res.message);
                    setData({ alert: false, id: 0, status: "" });
                    getList(1);
                }
            })
            .catch((error: any) => { });
    };

    return (
        <React.Fragment>
            <div className="page-content">
                <Container fluid>
                    <BreadCrumb title="Manage User" pageTitle="Manage User" location={"/admin/menu"}/>
                    <Row>
                        <Col lg={12}>
                            <Card id="leadsList">
                                <CardHeader className="border-0">
                                    <Row className="mb-4">
                                        <Col sm={4}>
                                            <div className="search-box">
                                                <Input
                                                    type="text"
                                                    className="form-control search"
                                                    placeholder="Search"
                                                    value={searchText}
                                                    onChange={(e) => setSearchText(e.target.value)}
                                                    onKeyUp={(e) => {
                                                        if (e.keyCode === 13) {
                                                            getList(1);
                                                        }
                                                    }}
                                                />
                                                <i className="ri-search-line search-icon"></i>
                                            </div>
                                        </Col>
                                        <div className="col-sm-auto ms-auto">
                                            <div className="hstack gap-2 flex-wrap">
                                                <button
                                                    type="button"
                                                    className="btn btn-brand-theme"
                                                    id="create-btn"
                                                    onClick={(e) => toggleModel("AddEditManageUser")}>
                                                    <i className="ri-add-line align-bottom me-1"></i>{" "}
                                                    Add
                                                </button>
                                            </div>
                                        </div>
                                    </Row>
                                    <div className="px-3 mt-3">
                                        <div className="table-card">
                                            <Grid
                                                headers={headers}
                                                rows={rows}
                                                count={totalCount}
                                                errorMessage={"No Data Found"}
                                                ShowLoader={showloader}
                                                onPageChange={onchange}
                                            />
                                        </div>
                                    </div>
                                </CardHeader>
                            </Card>
                        </Col>
                    </Row>
                </Container>
            </div>

            <AddEditAdminManageUser
                formData={formData.current}
                getlist={() => getList(1)}
                show={modelName === "AddEditManageUser" ? true : false}
                onCloseClick={() => {
                    setModelName("");
                }}
                parentId={id}
            />

            {/* Modal */}

            <Modal size="md" isOpen={data.alert} centered>
                <ModalHeader
                    className="mb-2"
                    toggle={() => setData({ alert: false, id: 0, status: "" })}>
                    Are you sure you want to do this action?
                </ModalHeader>
                <ModalFooter className="border-top p-2">
                    <div className="hstack gap-2 justify-content-end">
                        <button
                            type="button"
                            className="btn btn-light"
                            onClick={() => setData({ alert: false, id: 0, status: "" })}>
                            Cancel
                        </button>
                        <button
                            type="submit"
                            id="manage-user-modal-submit-btn"
                            className="btn btn-primary"
                            onClick={() => changeStatus(data.id, data.status)}>
                            Ok
                        </button>
                    </div>
                </ModalFooter>
            </Modal>
        </React.Fragment>
    );
};

export default AdminManageUser;
