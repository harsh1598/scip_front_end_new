...
import AddEditTrainingVideo from "./AddEditTrainingVideo";
import HelperService from "../../../../utility/HelperService";

const TrainingVideos = () => {

    document.title = `${ProjectName} | Training Videos`;
    ...
    const getlist = (page?: any, AcessData?: any) => {
        ...
        WebService.getAPI({
            action: `/training-videos-pagination-list`,
            body: obj,
        })
            .then((res: any) => {
                if (res && res.coloumns) {
                ...
                if (res && res.list) {
                    ...
                    for (var i in res.list) {
                        ...
                        for (var j in arr) {
                            ...
                            } else if (arr[j].db_column == "video_desc") {
                                columns.push({
                                    value: showDescriptionTruncate({
                                        video_description: res.list[i].video_desc,
                                    }),
                                });
                            } else if (arr[j].db_column == "added_date") {
                                columns.push({
                                    value: HelperService.getOrderDateFormat(res.list[i].added_date),
                                });
        ...
        return (
            <>
                <div className="flex-grow-1 overflow-hidden" style={{ width: "200px" }}>
                    <p className="text-truncate mb-0">{data.video_description}</p>
                </div>
            </>
        );
    };
                        ...
                        <UncontrolledDropdown>
                            ...
                            <DropdownMenu className="dropdown-menu-end">
                                {data.menueMoadule.is_edit == 1 && (
                                    <DropdownItem
                                        onClick={(e) => toggleModel("AddEditTrainingVideo", data.id)}>
    ...
    const toggleModel = (name: string, id?: any) => {
        if (id) {
            WebService.getAPI({
                action: "/training-videos-details/" + id,
            })
                .then((res: any) => {
                    if (res && res.result) {
    ...
    const changeStatus = (id: any) => {
        ...
        obj.id = id;
        WebService.postAPI({
            action: `/training-videos-update-status`,
            body: obj,
            id: "training-videos-modal-submit-btn",
        })
            .then((res: any) => {
                if (res) {
    ...
    return (
        <React.Fragment>
                                            ...
                                            <div className="col-sm-auto ms-auto">
                                                <div className="hstack gap-2 flex-wrap">
                                                    <button
                                                        ...
                                                        id="create-btn"
                                                        onClick={e => toggleModel('AddEditTrainingVideo')}
                                                    >
            ...
            <AddEditTrainingVideo
                formData={formData.current}
                getlist={() => getlist(1)}
                show={modelName === "AddEditTrainingVideo" ? true : false}
                onCloseClick={() => {
                    setModelName("");
                }}
            />
