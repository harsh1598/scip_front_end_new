...
import AddEditRoundOfInvestment from "./AddEditRoundOfInvestment";

const RoundOfInvestment = () => {
  document.title = `${ProjectName} | Round of investment`;
  ...
  const getlist = (page?: any, AcessData?: any) => {
    ...
    WebService.getAPI({
      action: `/round-of-investment-pagination-list`,
      body: obj,
    })
      .then((res: any) => {
        if (res && res.coloumns) {
                ...
                {AcessData.is_edit == 1 && (
                  <DropdownItem>
                    <a className="text-body"
                      onClick={() => toggleModel("AddEditRoundOfInvestment", data.roundofinvestmentId)}>
  ...
  const toggleModel = (name: string, id?: any) => {
    if (id) {
      WebService.getAPI({
        action: "/round-of-investment-details/" + id,
      })
        .then((res: any) => {
          if (res && res.result) {
  ...
  const onchangeStatus = (id: any, status: any) => {
    let obj: any = {};
    obj.roundofinvestmentId = id;
    obj.status = status;
    WebService.postAPI({
      action: `/round-ofinvestment-status`,
      body: obj,
    })
      .then((res: any) => {
        if (res) {
  ...
  return (
    <>
      <AddEditRoundOfInvestment
        formData={formData.current}
        getlist={() => getModuleAcesslist(1)}
        show={modelName === "AddEditRoundOfInvestment" ? true : false}
        onCloseClick={() => {
          setModelName("");
        }}
      />
      <div className="page-content">
        <Container fluid>
          <BreadCrumb
            title="Round of investment"
            pageTitle="Round of investment"
            location={"/admin/menu"}
          />
          <Row>
            <Col lg={12}>
                      ...
                      <div className="col-sm-auto ms-auto">
                        <div className="d-flex hstack gap-2 flex-wrap">
                          <button
                            ...
                            onClick={() =>
                              toggleModel("AddEditRoundOfInvestment")
                            }>
...
export default RoundOfInvestment;
