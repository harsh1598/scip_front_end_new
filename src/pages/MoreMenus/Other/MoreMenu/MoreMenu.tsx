...
import AddEditMoreMenu from "./AddEditMoreMenu";
...
const MoreMenu = () => {
    ...
    const getlist = (page?: any, AcessData?: any) => {
        ...
        WebService.getAPI({
            action: `/more-menu-pagination-list`,
            body: obj,
        })
            .then((res: any) => {
                if (res && res.coloumns) {
                    ...
                    for (var i in res.list) {
                        ...
                        for (var j in arr) {
                            if (arr[j].title == "Sr No.") {
                                ...
                                columns.push({
                                    value: HelperService.getOrderDateFormatWithTime(res.list[i].updated_date),
                                });
                        ...
                        <UncontrolledDropdown>
                            ...
                            <DropdownMenu className="dropdown-menu-end">
                                {data.menueMoadule.is_edit == 1 && (
                                    <DropdownItem
                                        onClick={(e) => toggleModel("AddEditMoreMenu", data.id)}>
                                    ...
                                    data.menueMoadule.sel_action_button.map((res: any) => {
                                        return (
                                            <>
                                                ...
                                                {res == "Sub Menus" && (
                                                    <DropdownItem>
                                                        {/* to={{pathname: `/admin/subMenus/${data.id}`,}} */}
                                                        <Link
                                                            className="text-body"
                                                            id="MoreSubMenu"
                                                            to={{pathname: `/admin/moremenu/submenu/${data.id}`}} >
                                                            <i className="ri-align-justify align-middle me-1"></i>
                                                            Sub Menus
                                                        </Link>
                                                    </DropdownItem>
    ...
    const changeStatus = (id: any) => {
        ...
        obj.id = id;
        WebService.postAPI({
            action: `/more-menu-update-status`,
            body: obj,
            id: "more-menu-modal-submit-btn",
        })
            .then((res: any) => {
                if (res) {
    ...
    const onclickGridRow = (data: any) => {
        ...
        if (array.length > 0) {
            WebService.postAPI({
                action: `/change-more-menu-seq-no`,
                body: {
                    list: array
                },
            }).then((res: any) => {
    ...
    return (
        <React.Fragment>
            <div className="page-content">
                <Container fluid>
                    <BreadCrumb
                        title="Manage More Menus"
                        pageTitle="Manage More Menus"
                        location={"/admin/menu"}
                    />
                    <Row>
                        <Col lg={12}>
                                            ...
                                            <div className="col-sm-auto ms-auto">
                                                <div className="hstack gap-2 flex-wrap">
                                                    <button
                                                        ...
                                                        id="create-btn"
                                                        onClick={e => toggleModel('AddEditMoreMenu')}
                                                    >
            ...
            <AddEditMoreMenu
                ...
                getlist={() => getlist(1)}
                show={modelName === "AddEditMoreMenu" ? true : false}
                onCloseClick={() => {
                    setModelName("");
                }}
            />
