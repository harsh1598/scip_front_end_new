...
const Application = () => {
  document.title = `${ProjectName} | Application`;
  ...
  const getlist = (page?: any, AcessData?: any) => {
    ...
    WebService.getAPI({
      action: `/application-pagination-list`,
      body: obj,
    })
      .then((res: any) => {
        if (res && res.coloumns) {
        ...
        if (res && res.list) {
          ...
          for (var i in res.list) {
            ...
            for (var j in arr) {
              ...
              } else if (arr[j].db_column == "show_note_before_submit") {
                columns.push({
                  value: showNoteBeforeSubmit({
                    show_note_before_submit: HelperService.getText(
                      res.list[i].show_note_before_submit,
                      80
                    ),
                  }),
                });
              } else if (arr[j].db_column == "is_app") {
                columns.push({
                  value: isshowIsAppbtn({
                    is_app: res.list[i].is_app,
                  }),
                });
              } else if (arr[j].title == "Action") {
                columns.push({
                  value: actionList({
                    id: res.list[i].applicationId,
            ...
            <UncontrolledDropdown>
              ...
              <DropdownMenu className="dropdown-menu-end">
                {data.menueMoadule.is_edit == 1 && (
                  <DropdownItem
                    onClick={(e) => toggleModel("AddEditApplication", data.id)}>
                        ...
                        {res == "Show Section" && (
                          <DropdownItem>
                            <Link
                              className="text-body"
                              to={{
                                pathname: `/admin/applicationSection/${data.id}`,
                              }}>
  ...
  const isshowIsAppbtn = (data: any) => {
    return (
      <>
        <span className={"badge badge-soft-success fs-12"}>
          {data.is_app === "Y" ? "APP" : "WEB"}
        </span>
      </>
    );
  };

  const showNoteBeforeSubmit = (data: any) => {
    return (
      <>
        <div className="flex-grow-1 overflow-hidden" style={{ width: "200px" }}>
          <span
            dangerouslySetInnerHTML={{ __html: data.show_note_before_submit }}
          />
        </div>
      </>
    );
  ...
  const changeStatus = (id: any, status: any) => {
    ...
    obj.status = status;
    WebService.postAPI({
      action: `/application-update-status`,
      body: obj,
      id: "application-modal-submit-btn",
    })
      .then((res: any) => {
        if (res) {
  ...
  return (
    <React.Fragment>
                    ...
                    <div className="col-sm-auto ms-auto">
                      <div className="hstack gap-2 flex-wrap">
                        <button
                          ...
                          id="create-btn"
                          onClick={(e) => toggleModel("AddEditApplication")}>
      ...
      <AddEditApplication
        ...
        show={modelName === "AddEditApplication" ? true : false}
        onCloseClick={() => {
          setModelName("");
        }}
      />
