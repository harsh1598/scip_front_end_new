...
import AddEditApplicationSubSection from "./AddEditApplicationSubSection";
...
const ApplicationSubSection = () => {
  document.title = `${ProjectName} | Application Sub Section`;
  ...
  const getlist = (page?: any) => {
    ...
    WebService.getAPI({
      action: `/application-sub-section-pagination-list`,
      body: obj,
    })
      .then((res: any) => {
        if (res && res.coloumns) {
        ...
        if (res && res.list) {
          ...
          for (var i in res.list) {
            ...
            for (var j in arr) {
              ...
              } else if (arr[j].db_column == "description") {
                columns.push({
                  value: description({
                    description: res.list[i].description,
                  }),
                });
              } else if (arr[j].title == "Action") {
                columns.push({
                  value: actionList({
                    id: res.list[i].subsectionId,
  ...
  const description = (data: any) => {
    return (
      <>
        <div className="flex-grow-1 overflow-hidden" style={{ width: "400px" }}>
          ...
          <li className="list-inline-item">
            <UncontrolledDropdown>
              ...
              <DropdownMenu className="dropdown-menu-end">
                <DropdownItem
                  onClick={(e) => toggleModel("AddEditApplicationSubSection", data.id)}>
                ...
                <DropdownItem>
                  <Link className="text-body" to={{ pathname: `/admin/questions/${data.id}` }}>
  ...
  const toggleModel = (name: string, id?: any, parentId?: any) => {
    if (id) {
      WebService.getAPI({
        action: "/application-sub-section-details/" + id,
      })
        .then((res: any) => {
          if (res && res.result) {
  ...
  const changeStatus = (id: any, status: any) => {
    ...
    obj.status = status;
    WebService.postAPI({
      action: `/application-sub-section-update-status`,
      body: obj,
      id: "application-sub-section-modal-submit-btn",
    })
      .then((res: any) => {
        if (res) {
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Application Sub Section" pageTitle="Application Sub Section" location={"/admin/menu"} />
          <Row>
            <Col lg={12}>
              <Card id="leadsList">
                    ...
                    <div className="col-sm-auto ms-auto">
                      <div className="hstack gap-2 flex-wrap">
                        <button
                          ...
                          id="create-btn"
                          onClick={(e) =>
                            toggleModel("AddEditApplicationSubSection", null, id)
                          }>
      ...
      <AddEditApplicationSubSection
        formData={formData.current}
        show={modelName === "AddEditApplicationSubSection" ? true : false}
        getlist={() => getlist(1)}
        onCloseClick={() => {
          setModelName("");
        }}
