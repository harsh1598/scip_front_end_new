...
import AddEditMoreMenuUser from "./AddEditMoreMenuUser";

const MoreMenuUser = () => {
    document.title = `${ProjectName} | User`;
    ...
    const getlist = (page?: any, AcessData?: any) => {
        ...
        WebService.getAPI({
            action: `/syndicate-user-pagination-list`,
            body: obj,
        })
            .then((res: any) => {
                if (res && res.coloumns) {
                        ...
                        <UncontrolledDropdown>
                            ...
                            <DropdownMenu className="dropdown-menu-end">
                                {data.menueMoadule.is_edit == 1 &&
                                    <DropdownItem
                                        onClick={(e) => toggleModel("AddEditUser", data.id)}>
                                ...
                                {data.menueMoadule.sel_action_button && data.menueMoadule.sel_action_button.length > 0 && data.menueMoadule.sel_action_button.map((res: any) => {
                                    return (<>
                                        ...
                                        {
                                            res == "Modules Column Access" && <DropdownItem>
                                                <Link className="text-body" to={{
                                                    pathname: `/admin/modules/index/${data.id}`,
                                                }}>
                                                    <i className=" ri-list-unordered  align-middle me-1"></i>{" "}
                                                    Modules Column Access
                                                </Link>
                                            </DropdownItem>
                                        }
    ...
    const toggleModel = (name: string, id?: any) => {
        if (id) {
            WebService.getAPI({
                action: "/syndicate-user-details/" + id,
            })
                .then((res: any) => {
                    if (res && res.result) {
                        formData.current = res.result;
                        setModelName(name);
                        setParentId(res.result.parent_id);
                    }
                })
                .catch((error: any) => { });
        } else {
    ...
    const changeStatus = (id: any) => {
        ...
        obj.userId = id;
        WebService.postAPI({
            action: `/syndicate-user-update-status`,
            body: obj,
            id: "syndicate-user-modal-submit-btn",
        })
            .then((res: any) => {
                if (res) {
    ...
    return (
        <React.Fragment>
                                        ...
                                        <div className="col-sm-auto ms-auto">
                                            <div className="hstack gap-2 flex-wrap">
                                                {/* <button
                                                    ...
                                                    id="create-btn"
                                                    onClick={(e) => toggleModel("AddEditUser")}>
            ...
            <AddEditMoreMenuUser
                ...
                getlist={() => getlist(1)}
                show={modelName === "AddEditUser" ? true : false}
                onCloseClick={() => {
                    setModelName("");
                }}
                parentId={parentId}
