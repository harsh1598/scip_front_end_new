...
import AddEditContactDirectory from "./AddEditContactDirectory";
import ContactDirectoryComment from "./ContactDirectoryComment";
import Grid, {
  GridColumn,
  GridRow,
...
const ContactDirectory = () => {
  document.title = `${ProjectName} | Contact Directory`;
  ...
  const getlist = (page?: any, AcessData?: any) => {
    ...
    WebService.getAPI({
      action: `/contact-directory-pagination-list`,
      body: obj,
    })
      .then((res: any) => {
        if (res && res.coloumns) {
        ...
        if (res && res.list) {
          ...
          for (var i in res.list) {
            ...
            for (var j in arr) {
              ...
              } else if (arr[j].db_column == "organization") {
                columns.push({
                  value: organizationColumns({
                    organizationData: res.list[i].organization,
                  }),
                });
              } else if (
  ...
  const nameColumns = (data: any) => {
    return (
      <>
        <div className="flex-grow-1 overflow-hidden" style={{ width: "200px" }}>
          <p className="text-truncate mb-0">{data.nameData}</p>
        </div>
      </>
    );
  };

  const organizationColumns = (data: any) => {
    return (
      <>
        <div className="flex-grow-1 overflow-hidden" style={{ width: "300px" }}>
          <p className="text-truncate mb-0">{data.organizationData}</p>
        </div>
      </>
    );
  };
          ...
          <li className="list-inline-item">
            <UncontrolledDropdown>
              ...
              {data.menueMoadule.is_edit == 1 && (
                <DropdownItem
                  onClick={(e) =>
                    toggleModel("AddEditContactDirectory", data.id)
                  }>
                  ...
                  data.menueMoadule.sel_action_button.map((res: any) => {
                    return (
                      <>
                        ...
                        {res == "Comment" && (
                          <DropdownItem
                          onClick={(e) => edit("ContactDirectoryComment", data.id)}>
                          <i className="ri-chat-2-line  align-middle me-1"></i>Comment
                        </DropdownItem>
                        )}
                      </>
                    );
  ...
  const toggleModel = (name: string, id?: any) => {
    if (id) {
      WebService.getAPI({
        action: "/contact-directory-details/" + id,
      })
        .then((res: any) => {
          if (res && res.result) {
  ...
  const _changeStatus = (id: any, status: any) => {
    ...
    obj.status = status;
    WebService.postAPI({
      action: `/contact-directory-status`,
      body: obj,
      id: "Contact-Directory-modal-submit-btn",
    })
      .then((res: any) => {
        if (res) {
  ...
  return (
    <React.Fragment>
                    ...
                    <div className="col-sm-auto ms-auto">
                      <div className="hstack gap-2 flex-wrap">
                        <button
                          ...
                          id="create-btn"
                          onClick={(e) =>
                            toggleModel("AddEditContactDirectory")
                          }>
      ...
      <AddEditContactDirectory
        ...
        show={modelName === "AddEditContactDirectory" ? true : false}
        onCloseClick={() => {
          setModelName("");
        }}
      />

      <ContactDirectoryComment
        id={id}
        setId={setId}
        getlist={() => getModuleAcesslist()}
        show={modelName === "ContactDirectoryComment" ? true : false}
        onCloseClick={() => {
          setModelName("");
          setId("");
        }}
