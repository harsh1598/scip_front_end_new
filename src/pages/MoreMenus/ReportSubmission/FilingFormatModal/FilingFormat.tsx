...
import AddEditFiling from "../FilingModal/AddEditFiling";
...
import AddEditFilingReportFormat from "./AddEditFilingReportFormat";

const FilingFormat = () => {
  document.title = `${ProjectName} |Manage Filing Progress Highlights`;
  ...
  const getlist = (page?: any, AcessData?: any) => {
    ...
    WebService.getAPI({
      action: `/filing-report-format-pagination-list`,
      body: obj,
    })
      .then((res: any) => {
        if (res && res.coloumns) {
                ...
                {AcessData.is_edit == 1 && (
                  <DropdownItem>
                    <a className="text-body" onClick={() =>
                      toggleModel("AddEditFilingReportFormat", data.formatId)}>
  ...
  const toggleModel = (name: string, id?: any) => {
    if (id) {
      WebService.getAPI({
        action: "/filing-report-format-details/" + id,
      })
        .then((res: any) => {
          if (res && res.result) {
  ...
  const onchangeStatus = (id: any, status: any) => {
    ...
    obj.status = status;
    WebService.postAPI({
      action: `/filing-report-format-status`,
      body: obj,
    })
      .then((res: any) => {
        if (res) {
  ...
  return (
    <>
      <AddEditFilingReportFormat
        formData={formData.current}
        getlist={() => getModuleAcesslist()}
        show={modelName === "AddEditFilingReportFormat" ? true : false}
        onCloseClick={() => {
          setModelName("");
        }}
      />
      <div className="page-content">
        <Container fluid>
          <BreadCrumb
            title="Manage Filing Progress Highlights"
            pageTitle="Manage Filing Progress Highlights"
            location={"/admin/menu"}
          />
          <Row>
            <Col lg={12}>
                    ...
                    <div className="col-sm-auto ms-auto">
                      <div className="d-flex hstack gap-2 flex-wrap">
                        <button
                          ...
                          onClick={() =>
                            toggleModel("AddEditFilingReportFormat")
                          }>
