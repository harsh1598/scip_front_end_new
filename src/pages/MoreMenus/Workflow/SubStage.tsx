...
import AddEditSubStage from "./AddEditSubStage";
import Grid, {
  GridColumn,
  GridHeader,
  GridRow,
...
//   },
//   {
//     title: "Duration of completion in days",
//     classTitle: "text-start",
//   },
//   {
//     title: "Sequence No",
...
const Stage = () => {
  document.title = `${ProjectName} | Sub Stage`;
  ...
  const getlist = (page?: any) => {
    ...
    obj.stageId = id;
    WebService.getAPI({
      action: `/substage-pagination-list`,
      body: obj,
    })
      .then((res: any) => {
        if (res && res.coloumns) {
        ...
        //     columns.push({ value: res.list[i].deal_tab });
          ...
          <li className="list-inline-item">
            <UncontrolledDropdown>
              ...
              <DropdownMenu className="dropdown-menu-end">
                <DropdownItem
                  onClick={(e) => toggleModel("AddEditSubStage", data.id)}>
                ...
                <DropdownItem>
                  <Link
                    className="text-body"
                    to={{
                      pathname: `/admin/workflow/checklist/${data.id}`,
                    }}>
                    <i className="ri-team-line align-middle me-1"></i>Workflow
                    CheckList
                  </Link>
                </DropdownItem>
                <DropdownItem>
                  <Link
                    className="text-body"
                    to={{
                      pathname: `/admin/workflow/template/${data.id}/${id}`,
                    }}>
                    <i className="ri-profile-line align-middle me-1"></i>
                    I-Manage Template
                  </Link>
                </DropdownItem>
                <DropdownItem>
                  <Link
                    className="text-body"
                    to={{
                      pathname: `/admin/workflow/document/${data.id}`,
                    }}>
                    <i className="ri-team-line align-middle me-1"></i>Workflow
                    Document
                  </Link>
                </DropdownItem>
                <DropdownItem onClick={(e) => toggleModel("LinkWorkflow")}>
                  <i className="ri-links-line align-middle me-1"></i>Link
                  Workflow SubStage
                  {/* <i className="ri-links-line align-middle me-1"></i>Link Workflow SubStage To Kanban SubStage */}
                </DropdownItem>
              </DropdownMenu>
            </UncontrolledDropdown>
          </li>
  ...
  const _changeStatus = (id: any, status: any) => {
    ...
    obj.status = status;
    WebService.postAPI({
      action: `/substage-update-status`,
      body: obj,
      id: "sub-stage-modal-submit-btn",
    })
      .then((res: any) => {
        // console.log(res)
        if (res) {
  ...
  return (
    <React.Fragment>
                    ...
                    <div className="col-sm-auto ms-auto">
                      <div className="hstack gap-2 flex-wrap">
                        <button
                          ...
                          id="create-btn"
                          onClick={(e) => toggleModel("AddEditSubStage")}>
      ...
      <AddEditSubStage
        ...
        stageId={id}
        getlist={() => getlist(1)}
        show={modelName === "AddEditSubStage" ? true : false}
        onCloseClick={() => {
          setModelName("");
        }}
      />
