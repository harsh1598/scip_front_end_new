...
import AddEditNotesCriteria from "./AddEditNotesCriteria";
...
const NotesCriteria = () => {
    ...
    document.title = `${ProjectName} | Notes Criteria`;
    ...
    const getlist = (page?: any) => {
        ...
        WebService.getAPI({
            action: `/question-criteria-pagination-list`,
            body: obj
        }).then((res: any) => {
            if (res && res.list) {
                ...
                for (var i in res.list) {
                    ...
                    columns.push({ value: res.list[i].criteria_title });
                    columns.push({ value: res.list[i].seq_no});
                    columns.push({ value: actionList({ id: res.list[i].criteriaId, active: res.list[i].active }) });
                    ...
                    <li className="list-inline-item">
                        <UncontrolledDropdown>
                            ...
                            <DropdownMenu className="dropdown-menu-end">
                                <DropdownItem onClick={e => toggleModel('AddEditNotesCriteria', data.id)}>
    ...
    const toggleModel = (name: string, id?: any) => {
        ...
        if (id) {
            WebService.getAPI({
                action: "/question-criteria-details/" + id,
            }).then((res: any) => {
                if (res && res.result) {
    ...
    const _changeStatus = (id: any, status: any) => {
        ...
        obj.status = status;
        WebService.postAPI({
            action: `/question-criteria-update-status`,
            body: obj,
            id: "question-modal-submit-btn"
        }).then((res: any) => {
            console.log(res)
            if (res) {
    ...
    return (
        <React.Fragment>
            <div className="page-content">
                <Container fluid>
                    <BreadCrumb title="Notes Criteria" pageTitle="Notes Criteria" location={"/admin/menu"} />
                    <Row>
                        <Col lg={12}>
                            <Card id="leadsList">
                                        ...
                                        <div className="col-sm-auto ms-auto">
                                            <div className="hstack gap-2 flex-wrap">
                                                <button
                                                    ...
                                                    id="create-btn"
                                                    onClick={e => toggleModel('AddEditNotesCriteria')}
                                                >
            ...
            <AddEditNotesCriteria
                formData={formData}
                setFormData={setFormData}
                questionId={id}
                getlist={() => getlist(1)}
                show={modelName === 'AddEditNotesCriteria' ? true : false}
                onCloseClick={() => {
                    setModelName("");
                    setFormData({});
                }}
            ...
            <Modal size="md" isOpen={data.alert} centered>
                ...
                <ModalFooter className="border-top p-2">
                    <div className="hstack gap-2 justify-content-end">
                        ...
                        <button type="submit" id="question-modal-submit-btn" className="btn btn-primary" onClick={() => _changeStatus(data.id, data.status)}>Ok</button>
                    </div>
                </ModalFooter>
            </Modal>
