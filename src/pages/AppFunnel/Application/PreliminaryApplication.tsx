...
const PreliminaryApplication = () => {
  ...
  document.title = `${ProjectName} | App Funnel - Application`;
    ...
    WebService.getAPI({
      ...
      .then((res: any) => {
        for (var i in res.result.sub_items) {
          if (res && res.result && res.result.sub_items[i].code == 'applications') {
  ...
  return (
    <>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb
            title="Preliminary Applications"
            pageTitle="Preliminary Applications"
          />
          <Row>
            <Col lg={12}>
              <Card>
                <CardBody>
                  ...
                  <TabContent activeTab={tabId} className="text-muted mt-4">
                    <TabPane
                      tabId="entrepreneur_applications"
                      id="#entrepreneur_app_funnel">
                      <Entrepreneur />
                    </TabPane>
                    <TabPane
                      tabId="investor_applications" id="#investor_app_funnel">
                      <Investor />
                    </TabPane>
                    <TabPane
                      tabId="sme_advisor_applications" id="#sme_advisor_app_funnel">
                      <SME />
                    </TabPane>
                  </TabContent>
                </CardBody>
