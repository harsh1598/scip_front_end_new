...
const StartupShortDetailsReport = (props: propData) => {
    const {
        handleSubmit,
        control,
        formState: { errors },
    ...
    const [startupShortDetails, setStartupShortDetails] = useState<any>([])
    ...
    const onClickExport = (data: any) => {
        if (props.acessMenuData.is_export == 1 && props.acessMenuData.is_opt_export == 0) {
            ...
            setStartupShortDetails(data)
        setUserOtp("")
        var obj: any = {}
        obj.type = 'StartupShortDetailsView'
    ...
    const sendOtp = (data?:any) => {
        ...
        if (props.acessMenuData.is_export == 1) {
            ...
            if (data && data?.date && data?.date[1]) {
                ...
                if (startupShortDetails && startupShortDetails?.date && startupShortDetails?.date[0]) {
                    obj.start_date = moment(startupShortDetails?.date[0]).format('DD/MM/YYYY');
                }
                if (startupShortDetails && startupShortDetails?.date && startupShortDetails?.date[1]) {
                    obj.end_date = moment(startupShortDetails?.date[1]).format('DD/MM/YYYY');
                }
            }
        }
        WebService.postAPI({
            action: "/report/startup-details-view-report-excel-url",
            body: obj
        }).then((res: any) => {
            if (res) {
...
export default StartupShortDetailsReport;
