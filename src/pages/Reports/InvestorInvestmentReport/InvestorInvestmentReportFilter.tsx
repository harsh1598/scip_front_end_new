...
const InvestorInvestmentReportFilter = (props: propData) => {
    const {
        handleSubmit,
        control,
        formState: { errors },
    ...
    const onClickExport = (data?: any) => {
        ...
        setUserOtp("")
        var obj: any = {}
        obj.type = 'InvestorInvestmentReport'
        WebService.postAPI({
    ...
    const sendOtp = (data?:any) => {
        ...
        if (props.acessMenuData.is_export == 1) {
            ...
            } if (data && data?.investorid) {
                obj.investorid = data?.investorid;
            } else if(props.acessMenuData.is_export == 1 && props.acessMenuData.is_opt_export == 1) {
                if (investorData && investorData?.date && investorData?.date[0]) {
                    obj.start_date = moment(investorData?.date[0]).format('DD/MM/YYYY');
                }
                if (investorData && investorData?.date && investorData?.date[1]) {
                    obj.end_date = moment(investorData?.date[1]).format('DD/MM/YYYY');
                }
                if (investorData && investorData?.investorid) {
                    obj.investorid = investorData?.investorid;
                }
            }
        }
        WebService.postAPI({
            action: "/report/investor-investment-report-excel-url",
            body: obj
        }).then((res: any) => {
            if (res) {
        ...
        >
            <form onSubmit={handleSubmit(onClickExport)} className="d-flex flex-column justify-content-end h-100">
                <OffcanvasHeader className="bg-light" toggle={props.onCloseClick}>
                    Investor Investment Report Filter
                </OffcanvasHeader>
                <OffcanvasBody >
                    <div>
                        <Label
...
export default InvestorInvestmentReportFilter;
