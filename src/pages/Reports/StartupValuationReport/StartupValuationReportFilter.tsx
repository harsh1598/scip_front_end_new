...
const StartupValuationReportFilter = (props: propData) => {
    const {
        handleSubmit,
        control,
        formState: { errors },
    ...
    const [startupValuationData, setStartupValuationData] = useState<any>([])
    ...
    const onClickExport = (data:any) => {
        ...
        setStartupValuationData(data)
        setUserOtp("")
        var obj: any = {}
        obj.type = 'StartupValuationView'
        WebService.postAPI({
    ...
    const sendOtp = (data?:any) => {
        ...
        if (props.acessMenuData.is_export == 1) {
            ...
            if (data && data?.date && data?.date[1]) {
                ...
                if (startupValuationData && startupValuationData?.date && startupValuationData?.date[0]) {
                    obj.start_date = moment(startupValuationData.current?.date[0]).format('DD/MM/YYYY');
                }
                if (startupValuationData && startupValuationData?.date && startupValuationData?.date[1]) {
                    obj.end_date = moment(startupValuationData?.date[1]).format('DD/MM/YYYY');
                }
            }
        }
        WebService.postAPI({
            action: "/report/startup-valuation-report-excel-url",
            body: obj
        }).then((res: any) => {
            if (res) {
        ...
        >
            <form onSubmit={handleSubmit(onClickExport)} className="d-flex flex-column justify-content-end h-100">
                <OffcanvasHeader className="bg-light" toggle={props.onCloseClick}>
                    Startup Valuation Report Filter
                </OffcanvasHeader>
                <OffcanvasBody >
                    <div>
                        <Label
...
export default StartupValuationReportFilter;
