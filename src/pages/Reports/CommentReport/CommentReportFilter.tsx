...
const CommentReportFilter = (props: propData) => {
    const {
        handleSubmit,
        control,
        formState: { errors },
    ...
    const [commentReport, setCommentReport] = useState<any>([]);
    ...
    const sendOtp = (data?:any) => {
        ...
        if (props.acessMenuData.is_export == 1) {
            ...
            if (data.campaignId) {
                ...
                if (commentReport && commentReport?.date && commentReport?.date[0]) {
                    obj.start_date = moment(commentReport.current?.date[0]).format('DD/MM/YYYY');
                }
                if (commentReport && commentReport?.date && commentReport?.date[1]) {
                    obj.end_date = moment(commentReport?.date[1]).format('DD/MM/YYYY');
                }
                if (commentReport && commentReport.companyId) {
                    obj.companyId = commentReport.companyId;
                }
                if (commentReport && commentReport.campaignId) {
                    obj.campaignId = commentReport.campaignId;
                }
            }
        }
        WebService.postAPI({
            action: "/report/comment-report-excel-url",
            body: obj
        }).then((res: any) => {
            if (res) {
...
export default CommentReportFilter;
