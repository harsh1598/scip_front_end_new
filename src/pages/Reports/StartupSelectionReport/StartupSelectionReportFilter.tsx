...
const StartupSelectionReportFilter = (props: propData) => {
    const {
        handleSubmit,
        control,
        formState: { errors },
    ...
    const [startupSelectionData, setStartupSelectionData] = useState<any>([])
    ...
    const onClickExport = (data:any) => {
        ...
        setStartupSelectionData(data)
        setUserOtp("")
        var obj: any = {}
        obj.type = 'StartupSelection'
        WebService.postAPI({
    ...
    const sendOtp = (data?:any) => {
        ...
        if (props.acessMenuData.is_export == 1) {
            ...
            if (data && data?.date && data?.date[1]) {
                ...
                if (startupSelectionData && startupSelectionData?.date && startupSelectionData?.date[0]) {
                    obj.start_date = moment(startupSelectionData.current?.date[0]).format('DD/MM/YYYY');
                }
                if (startupSelectionData && startupSelectionData?.date && startupSelectionData?.date[1]) {
                    obj.end_date = moment(startupSelectionData?.date[1]).format('DD/MM/YYYY');
                }
            }
        }
        WebService.postAPI({
            action: "/report/startup-selection-report-excel-url",
            body: obj
        }).then((res: any) => {
            if (res) {
        ...
        >
            <form onSubmit={handleSubmit(onClickExport)} className="d-flex flex-column justify-content-end h-100">
                <OffcanvasHeader className="bg-light" toggle={props.onCloseClick}>
                    Startup Selection Report Filter
                </OffcanvasHeader>
                <OffcanvasBody >
                    <div>
                        <Label htmlFor="leads-tags" className="form-label text-muted text-uppercase fw-semibold mb-3"> Filters</Label>
                        <Row>
                            <Col lg={12}>
                                <div className="mb-3">
...
export default StartupSelectionReportFilter;
