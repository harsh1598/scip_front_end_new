...
const StartupDetailsReportFilter = (props: propData) => {
    const {
        handleSubmit,
        control,
        formState: { errors },
    ...
    const onClickExport = (data?: any) => {
        if (props.acessMenuData.is_export == 1 && props.acessMenuData.is_opt_export == 0) {
            ...
            startupData.current = data
            setUserOtp("")
            var obj: any = {}
            obj.type = 'StartupDetailsView'
            WebService.postAPI({
    ...
    const sendOtp = (data?: any) => {
        ...
        if (props.acessMenuData.is_export == 1) {
            ...
            if (data && data?.date && data?.date[1]) {
                ...
                if (startupData.current && startupData?.current?.date && startupData.current?.date[0]) {
                    obj.start_date = moment(startupData.current?.date[0]).format('DD/MM/YYYY');
                }
                if (startupData.current && startupData.current?.date && startupData.current?.date[1]) {
                    obj.end_date = moment(startupData.current?.date[1]).format('DD/MM/YYYY');
                }
            }
        }
        WebService.postAPI({
...
export default StartupDetailsReportFilter;
