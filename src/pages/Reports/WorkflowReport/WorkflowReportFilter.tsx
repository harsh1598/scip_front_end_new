...
const WorkflowReportFilter = (props: propData) => {
    const {
        handleSubmit,
        control,
        formState: { errors },
    ...
    const [workflowReport, setWorkflowReport] = useState<any>([]);
    ...
    const sendOtp = (data?:any) => {
        ...
        if (props.acessMenuData.is_export == 1) {
            ...
            if (data && data.campaignId) {
                ...
                if (workflowReport && workflowReport?.date && workflowReport?.date[0]) {
                    obj.start_date = moment(workflowReport?.date[0]).format('DD/MM/YYYY');
                }
                if (workflowReport && workflowReport?.date && workflowReport?.date[1]) {
                    obj.end_date = moment(workflowReport?.date[1]).format('DD/MM/YYYY');
                }       if (workflowReport && workflowReport.companyId) {
                    obj.companyId = workflowReport.companyId;
                }
                if (workflowReport && workflowReport.campaignId) {
                    obj.campaignId = workflowReport.campaignId;
                }
            }
        }
        WebService.postAPI({
            action: "/report/workflow-report-excel-url",
            body: obj
        }).then((res: any) => {
            if (res) {
