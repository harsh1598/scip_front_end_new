...
const HorizontalLayout = (props) => {
    ...
    navData.forEach(function (value, key) {
        ...
        if (key >= menuSplitContainer) {
            let val = value;
            val.childItems = value.sub_items;
            val.isChildItem = (value.sub_items) ? true : false;
            delete val.sub_items;
            splitMenuItems.push(val);
        } else {
            menuItems.push(value);
        }
    });
    menuItems.push({ id: 'more', label: 'More', icon: 'ri-briefcase-2-line', link: "/#", stateVariables: isMoreMenu, sub_items: splitMenuItems, click: function (e) { e.preventDefault(); setIsMoreMenu(!isMoreMenu); }, });
                        ...
                        {!item['isHeader'] ?
                            (item.sub_items && item.sub_items.length > 0 ? (
                                <li className="nav-item">
                                    ...
                                    <Collapse
                                        className={item.id === "baseUi" && item.sub_items.length > 13 ? "menu-dropdown mega-dropdown-menu" : "menu-dropdown"}
                                        isOpen={item.stateVariables}
                                        id="sidebarApps">
                                        {/* subItms  */}
                                        {item.id === "baseUi" && item.sub_items.length > 13 ? (
                                            <React.Fragment>
                                                <Row>
                                                    {item.sub_items && ((item.sub_items || []).map((subItem, key) => (
                                                        <React.Fragment key={key}>
                                                            {key % 2 === 0 ? (
                                                                <Col lg={4}>
                                                                    <ul className="nav nav-sm flex-column">
                                                                        <li className="nav-item">
                                                                            <Link to={item.sub_items[key].link} className="nav-link">{item.sub_items[key].label}</Link>
                                                                        </li>
                                                                    </ul>
                                                                </Col>
                                                            ) : (
