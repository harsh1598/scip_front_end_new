...
const VerticalLayout = (props) => {
    ...
    */
    ...
    useEffect(() => {
        ...
        if (props.layoutType === "vertical" || props.layoutType === "semibox") {
            initMenu();
        }
        // let url = window.location.href;
        // if(url === "http://localhost:3000/renewals"){
    ...
    return (
        <React.Fragment>
            {/* <Loader show={navData.length == 0 ? true : false} /> */}
            {/* menu Items */}
                            ...
                            : (
                                (item.sub_items && item.sub_items.length > 0 ? (
                                    <li className="nav-item">
                                        ...
                                        <Collapse
                                            ...
                                            <ul className="nav nav-sm flex-column test">
                                                {/* subItms  */}
                                                {item.sub_items && ((item.sub_items || []).map((sub_items, key) => (
                                                    <React.Fragment key={key}>
                                                        {!sub_items.isChildItem ? (
                                                            <li className="nav-item">
                                                                <Link
                                                                    to={sub_items.link ? sub_items.link : '/#'}
                                                                    className="nav-link"
                                                                >
                                                                    {props.t(sub_items.label)}
                                                                    {sub_items.badgeName ?
                                                                        <span className={"badge badge-pill bg-" + sub_items.badgeColor} data-key="t-new">{sub_items.badgeName}</span>
                                                                        : null}
                                                                </Link>
                                                            </li>
                                                        ) : (
                                                            <li className="nav-item">
                                                                ...
                                                                <Collapse className="menu-dropdown" isOpen={sub_items.stateVariables} id="sidebarEcommerce">
                                                                    <ul className="nav nav-sm flex-column">
                                                                        {/* child subItms  */}
                                                                        {sub_items.childItems && (
                                                                            (sub_items.childItems || []).map((childItem, key) => (
                                                                                <React.Fragment key={key}>
                                    ...
                                    <li className="nav-item" style={{ position: 'relative' }}>
                                        ...
                                        <div className='ms-2'>
                                            <span className='multi-popver'><i className='ri-information-line' id='PopoverDismissible' style={{ cursor: 'pointer' }}></i></span>
                                            <UncontrolledPopover placement="right" target="PopoverDismissible" >
                                                ...
                                                <PopoverBody>
                                                    ...
                                                    <div className='mt-3'>
                                                        <Button type="button" className="btn btn-success waves-effect waves-light me-3">Next</Button>
                                                        <Button type="button" className="btn btn-danger waves-effect waves-light">Cancel</Button>
                                                    </div>
                                                </PopoverBody>
            ...
            <li className="nav-item">
                <Link className="nav-link menu-link" onClick={(e) => toggleModel("Helpcenter")}>
                    <i className='ri-headphone-line'></i> <span >Help Center</span>
                </Link>
            </li>
